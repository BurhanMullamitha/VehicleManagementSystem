...
public class TreeSetClient {
	...
	public static TreeSetInterface treesetInt;
	...
	JLabel vehicle_id = new JLabel("0");
	JLabel vehicle_make = new JLabel("0");
	JLabel vehicle_price = new JLabel("0");
	...
	private final JLabel label_2 = new JLabel("Vehicle");
	private final JLabel label_3 = new JLabel("Management");
	private final JLabel label_4 = new JLabel("System");
	private final JLabel label_5 = new JLabel("<-- Click on the Links ");
	...
	private final JLabel label_7 = new JLabel("Kg");
	private final JLabel label_9 = new JLabel("Litres");
	private final JLabel label_20 = new JLabel("L/100Km");
	private final JLabel label_21 = new JLabel("EURO");
	...
	public static void connectRMIServer() throws MalformedURLException, RemoteException, NotBoundException {
		...
		treesetInt = (TreeSetInterface) Naming.lookup(url + "set");
	}
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					TreeSetClient window = new TreeSetClient();
...
//			System.exit(0);
		...
		JLabel label_22 = new JLabel("Kg");
		label_22.setBounds(235, 310, 61, 16);
		deletePanel.add(label_22);

		JLabel label_23 = new JLabel("Litres");
		label_23.setBounds(235, 360, 61, 16);
		deletePanel.add(label_23);

		JLabel label_24 = new JLabel("L/100Km");
		label_24.setBounds(235, 410, 61, 16);
		deletePanel.add(label_24);

		JLabel lblEuro_2 = new JLabel("EURO");
		lblEuro_2.setBounds(311, 460, 61, 16);
		deletePanel.add(lblEuro_2);
		...
		btnDeleteVehicle.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					treesetInt.deleteVehicle(vehicleID);
			...
			public void itemStateChanged(ItemEvent e) {
				if (e.getStateChange() == ItemEvent.SELECTED) {
					try {
						...
						TreeNode selectedVehicle = treesetInt.getVehicleInfo(idprovider);
		...
		lblPrice_2.setBounds(129, 265, 61, 16);
		...
		vehicle_price.setBounds(183, 265, 69, 16);
		...
		label_6.setBounds(284, 195, 61, 16);

		searchVehiclePanel.add(label_6);
		lblLkm.setBounds(284, 237, 61, 16);

		searchVehiclePanel.add(lblLkm);
		lblEuro.setBounds(260, 265, 61, 16);

		searchVehiclePanel.add(lblEuro);
		lblKg.setBounds(392, 97, 22, 16);

		searchVehiclePanel.add(lblKg);
		...
		btnNext.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (currentVehicleIndex != vehicleIDArray.size() - 1) {
					...
					TreeNode currentVehicleInfo;
					try {
						currentVehicleInfo = treesetInt.searchByID(vehicleIDArray.get(currentVehicleIndex));
		...
		label_7.setBounds(392, 320, 61, 16);

		insertPanel.add(label_7);
		label_9.setBounds(315, 367, 61, 16);

		insertPanel.add(label_9);
		label_20.setBounds(315, 408, 61, 16);

		insertPanel.add(label_20);
		label_21.setBounds(392, 470, 61, 16);

		insertPanel.add(label_21);
		...
		btnInsertVehicle.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					treesetInt.insert(vehicleID, make, model, engine, year, doors, seats, weight, price, fuelCapacity,
							fuelAverage);
		...
		label_8.setBounds(315, 367, 61, 16);

		modifyPanel.add(label_8);
		lblLkm_1.setBounds(315, 408, 61, 16);

		modifyPanel.add(lblLkm_1);
		lblEuro_1.setBounds(392, 470, 61, 16);

		modifyPanel.add(lblEuro_1);
		...
		btnModifyVehicle.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					TreeNode updateVehicleInfo = new TreeNode(vehicleID, make, model, engine, year, doors, seats,
							weight, price, fuelCapacity, fuelAverage);
					treesetInt.updateVehicle(updateVehicleInfo);
		...
		label_5.setBounds(65, 275, 420, 39);

		homePanel.add(label_5);
		...
		JLabel OpeninTreeSet = new JLabel("Open Program in Linked List");
		OpeninTreeSet.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				LinkedListClient linkedListClient = new LinkedListClient();
				frame.setVisible(false);
				linkedListClient.frame.setVisible(true);
			}
		});
		OpeninTreeSet.setForeground(Color.WHITE);
		OpeninTreeSet.setFont(new Font("Lucida Grande", Font.BOLD, 14));
		OpeninTreeSet.setBounds(15, 559, 216, 29);
		...
		label_1.setBounds(47, 74, 85, 58);

		navigationPanel.add(label_1);
		label_2.setForeground(Color.WHITE);
		label_2.setBounds(144, 74, 88, 16);

		navigationPanel.add(label_2);
		label_3.setForeground(Color.WHITE);
		label_3.setBounds(144, 94, 88, 16);

		navigationPanel.add(label_3);
		label_4.setForeground(Color.WHITE);
		label_4.setBounds(144, 116, 88, 16);

		navigationPanel.add(label_4);
...
//				
	...
	public void setVehicleID() {
		try {
			int max_id = treesetInt.getMaxVehicleID();
	...
	public void getAllVehicleIDs() {
		try {
			...
			Set<TreeNode> idList = treesetInt.getAllIDs();
			Iterator<TreeNode> iterator = idList.iterator();
...
//			JOptionPane.showMessageDialog(null, e);
	...
	public void setSearchValues(String vehicleProperty) {
		...
		try {
			...
			Set<TreeNode> VehicleList = null;

			if (vehicleProperty.equals("vehicleID")) {
				VehicleList = treesetInt.allVehicleData();
			} else if (vehicleProperty.equals("vehicleMake")) {
				VehicleList = treesetInt.searchByMake(makeSearchCB.getSelectedItem().toString());
			} else if (vehicleProperty.equals("vehiclePrice")) {
				VehicleList = treesetInt.searchByPrice(Integer.parseInt(minPriceSearchTF.getText()),
						Integer.parseInt(maxPriceSearchTF.getText()));
			}

			for (TreeNode node : VehicleList) {
			...
			TreeNode currentVehicleInfo = null;
			try {
				currentVehicleInfo = treesetInt.searchByID(vehicleIDArray.get(currentVehicleIndex));
			} catch (Exception e) {
